...
save_as_image(x = ft, path = "~/chinook_growth/figures/stock_table.png")
...
char <- read.csv("transformed_data/stock_characteristics_data.csv")
...
d$`Release Region` <- plyr::mapvalues(d$release_location_rmis_region, 
                                      c("LOCR","UPCR","CECR","NWC","NOOR","SKAG"), 
                                      c("Lower Columbia River", "Upper Columbia river", 
                                        "Central Columbia river", "Washington Coast", 
                                        "Northern Oregon","Skagit River"))

d$`Release Basin (RMIS code)` <- d$release_location_rmis_basin
d$Group <- d$group
ft <- flextable(d,
                col_keys = c("Group", "Run", "Release Age","Ocean Dsn", 
                             "Sex", "Release Region",
                             "Release Basin (RMIS code)", "Number")) %>%
  vline(j="Group")%>%
  merge_v(j = c("Group")) %>%
  hline(j = "Group") %>%
  bg(i = ~Group %% 2== 0 , bg = rgb(0.9,0.9,0.9), part = "body") %>%
...
ft <- autofit(ft)
#ft <- tempfile(fileext = ".png")
save_as_image(x = ft, path = "~/chinook_growth/figures/stock_chars_table.png")
