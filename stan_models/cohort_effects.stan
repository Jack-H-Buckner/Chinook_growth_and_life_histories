...
transformed parameters {
  ...
  for(i in 1:N) {
    pred[i] = release_length[i] + RE[brood_year[i]+1]; // starting length
    for(a in release_age[i]:(age[i])) {
        pred[i] = pred[i] + (1.0 - slope[stock[i]]*a) * growth_inc[brood_year[i]+a, stock[i]]; // add year effects 
    }
  }
